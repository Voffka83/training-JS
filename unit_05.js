...
// в.out - 2 строку вида:
// 12_14_16_18_20_ .. 36_38_  Разделитель - нижнее подчеркивание. 
...
function t2() {
	...
	for (let i = 12; i < 40; i = i + 2) {
		x1 += i + '_';
		document.querySelector('.out-2').innerHTML = x1;
	}
}
...
// в.out - 3 строку вида:  25_24_23_22_ .. 8_7_
// Разделитель - нижнее подчеркивание. Задача решается с помощью цикла.
...
function t3() {
	...
	for (let i = 25; i > 6; i--) {
		x1 += i + '_';
		document.querySelector('.out-3').innerHTML = x1;
	}
}
...
// строку вида:  77_74_71_68_65_62_ ... _38_35_
...
function t4() {
	...
	document.querySelector('.out-4').innerHTML = x1;
...
// строку вида:  1_*2_**3_*4_** ..16_**17_*
...
function t5() {
	...
	for (i = 1; i <= 17; i++) {
		...
		document.querySelector('.out-5').innerHTML = x1;
	}
}
...
//  Task 6
// Кнопка .b-6 запускает функцию t6. Функция должна выводить 
// в.out - 6 строку вида:
//
// ******<br>
// ******<br>
...
// 6 звездочек.Перенос строки - br.Количество строк(итераций, повторений) 
...
function t6() {
	...
	document.querySelector('.out-6').innerHTML = x6;
}
...
// которая выводит в.out - 7 числа от введенного пользователем до нуля 
// включительно. Разделитель - знак подчеркивания. 
// Если пользователь ввел 4 и нажал кнопку, мы получим:  4_3_2_1_0_
// Задача решается с помощью цикла.

function t7() {
	let x7 = ' ';
	let num7 = +document.querySelector('.i-7').value;
	for (i = num7; i >= 0; i--) {
		x7 += i + '_';
	}
	document.querySelector('.out-7').innerHTML = x7;
}
...
// которая выводит в.out - 8 числа от первого введенного до второго 
// включительно, с шагом 1. Разделитель подчеркивание. 
...
function t8() {
	...
	for (i = num81; i <= num82; i++) {
		x8 += i + '_';

	} document.querySelector('.out-8').innerHTML = x8;
}
...
// которая выводит в.out - 9 числа от меньшего введенного до большего 
// включительно, с шагом 1. Разделитель - подчеркивание. 
// Если пользователь ввел 4 и 8  и нажал кнопку, мы получим:
// 4_5_6_7_8_
...
// четные годы от 1950 до 1970 включительно.
...
// перебрать их с помощью цикла. Обращение к div выглядит 
// так divs[i].innerHTML вывести в .out-11 содержимое каждого блока. 
// Разделитель - знак подчеркивания. В результате должно получиться так:
//     one_3_4_two_
...
function t13() {
	const div13 = document.querySelectorAll('.i-13');
	for (i = 0; i <= div13.length; i++) {
		div13[i].value = i + 1;
	}
}
...
// получить все input.i-14 перебрать их с помощью цикла. 
// Обращение к элементу выглядит так elem[i] вывести в .out-14 
// value выбранного. Проверить выбран ли элемент можно с
...
//  Task 15
// Кнопка .b-15 запускает функцию t15  Функция должна выводить 
// следующую последовательность в.out - 15:
